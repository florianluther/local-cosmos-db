import { Container, CosmosClient, Database, StoredProcedure } from '@azure/cosmos'
import { Conflict, Created, NoContent, NotFound, Ok } from '../result'
import { IRepositoryConfiguration } from './configuration'
import { IQuery, IRepository, IRepositoryQueryResult, IStoredProcedure } from './repository'
import { CosmosDbDocument, ICosmosDbOptions, ICosmosDbQueryOptions } from './types'
    ...
    implements IRepository<T>, IQuery<ICosmosDbQueryOptions, T>, IStoredProcedure
{
    ...
    async delete(id: string, partitionKey: string): Promise<NoContent | NotFound> {
        try {
    ...
    async update(entity: T, id: string, partitionKey: string): Promise<Ok<T> | NotFound> {
        try {
            const database: Database = this.client.database(this.databaseName)
            const container: Container = database.container(this.containerName)
     ...
     * @memberof CosmosDbRepository
     */
    async query(options: ICosmosDbQueryOptions): Promise<Ok<IRepositoryQueryResult<T>>> {
        const database: Database = this.client.database(this.databaseName)
        const container: Container = database.container(this.containerName)
